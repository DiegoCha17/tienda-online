...
export default function AdminCategoryManager() {
  ...
  const deleteCategory = async (id: number, categoryName: string) => {
    if (
      !confirm(
        `¿Eliminar la categoría "${categoryName}"? Los productos pasarán a "General".`,
      )
    )
      return;
    try {
  ...
  return (
    <div className="bg-gray-50 rounded-3xl p-8 mb-8 shadow-sm border border-gray-100 ">
      {" "}
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8 w-full">
        ...
        <input
          ...
          className="flex-1 w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 text-gray-900 focus:bg-gray-50 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none"
        />{" "}
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {categories.map((category) => (
          <div
            key={category.id}
            className="group bg-gray-50 border border-gray-100 rounded-xl p-4 flex justify-between items-center hover:bg-gray-50 hover:border-gray-200 hover:shadow-sm transition-all"
          >
            {" "}
