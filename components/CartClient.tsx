...
export default function CartClient({ whatsappNumber }: CartClientProps) {
  ...
  const submitOrder = async (values: FormValues) => {
    ...
    try {
      ...
      cart.forEach(item => {
        ...
        if (item.selectedSpecs) {
          const specs = Object.entries(item.selectedSpecs)
            .map(([k, v]) => `${k}: ${v}`)
            .join(', ');
          if (specs) specStr = ` [${specs}]`;
        }
        message += `${item.quantity}x ${item.name}${specStr} (${formatCRC(item.price * item.quantity)})\n`;
      });
      ...
      toast.success('¡Pedido procesado de forma segura! Redirigiendo a WhatsApp...', { duration: 5000 });
      localStorage.removeItem('cart');
      setCart([]);
      setShowForm(false);
      reset();
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 py-12 px-4 sm:px-6 lg:px-8 transition-colors">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-8 tracking-tight">Tu Carrito de Compras</h1>

        {cart.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm p-16 text-center space-y-6 border border-slate-100 dark:border-slate-800">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              {cart.map((item) => (
                <div
                  ...
                  className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-300 p-5 sm:p-6 flex flex-col sm:flex-row gap-6 items-center border border-slate-100 dark:border-slate-800 transform hover:-translate-y-1"
                >
                  <img
                    src={item.image_url}
                    alt={item.name}
                    className="w-32 h-32 rounded-2xl object-cover shadow-sm bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700"
                  />
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    {item.selectedSpecs && Object.keys(item.selectedSpecs).length > 0 && (
                      <div className="flex flex-wrap gap-2 justify-center sm:justify-start mb-2">
                        {Object.entries(item.selectedSpecs).map(([k, v], idx) => (
                          <span key={idx} className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-md font-medium border border-gray-200">
                            {k}: {v}
                          </span>
                        ))}
                      </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl shadow-indigo-500/5 p-8 border border-slate-100 dark:border-slate-800 sticky top-24">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-6">Resumen</h3>
