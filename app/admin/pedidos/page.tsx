...
async function getOrders() {
  const orders =
    await sql` SELECT id, customer_name, customer_email, customer_phone, customer_address, total, status, created_at FROM orders ORDER BY created_at DESC `;
  return orders;
}
async function getOrderItems(orderId: number) {
  const items =
    await sql` SELECT product_name, product_price, quantity, subtotal FROM order_items WHERE order_id = ${orderId} ORDER BY id ASC `;
  return items;
}
